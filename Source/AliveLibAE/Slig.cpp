...
void Slig::Motion_6_Shoot()
{
    if (GetAnimation().GetIsLastFrame())
    {
        if (sControlledCharacter == this && mHealth > FP_FromInteger(0))
        {
            if (Input().IsAnyHeld(InputCommands::Enum::eThrowItem))
            {
                ...
                // Recoil right
                if (GetAnimation().GetFlipX() && Input().IsAnyHeld(InputCommands::Enum::eRight))
                {
                    PathLine* pLine = nullptr;
                    FP hitX = {};
                    FP hitY = {};
                ...
                // Recoil left
                if (!GetAnimation().GetFlipX() && Input().IsAnyHeld(InputCommands::Enum::eLeft))
                {
                    PathLine* pLine = nullptr;
                    FP hitX = {};
                    FP hitY = {};
                ...
                // General recoil
                if (!Input().IsAnyHeld(InputCommands::Enum::eDown) || GetSpriteScale() != FP_FromDouble(0.5))
                {
...
void Slig::Motion_8_SlidingToStand()
{
    ...
    else
    {
        ...
        if (mCurrentMotion == eSligMotions::Motion_8_SlidingToStand)
        {
            ...
            {
                if ((GetAnimation().GetFlipX() && Input().IsAnyHeld(InputCommands::Enum::eRight)) || ((!(GetAnimation().GetFlipX()) && Input().IsAnyHeld(InputCommands::Enum::eLeft))))
                {
...
void Slig::Motion_42_ShootZ()
{
    if (GetAnimation().GetIsLastFrame())
    {
        ...
        if (sControlledCharacter == this && (!Input().IsAnyHeld(InputCommands::Enum::eThrowItem) || !Input().IsAnyHeld(InputCommands::Enum::eDown)))
        {
            SND_SEQ_PlaySeq(SeqId::HitBottomOfDeathPit_9, 1, 1);
...
s16 Slig::ToShootZ()
{
    if (Input().IsAnyHeld(InputCommands::Enum::eThrowItem) && GetSpriteScale() == FP_FromDouble(0.5) && !GetTeleporting())
    {
...
{
    if (!Input().IsAnyHeld(InputCommands::Enum::eDown) || GetSpriteScale() != FP_FromDouble(0.5) || GetTeleporting())
    {
        mCurrentMotion = eSligMotions::Motion_6_Shoot;
    }
    else
...
s16 Slig::HandlePlayerControlled()
{
    ...
    if (Input().IsAnyHeld(InputCommands::Enum::eFartOrRoll))
    {
    ...
    else if (Input().IsAnyPressed(InputCommands::Enum::eFartOrRoll | InputCommands::Enum::eHop))
    {
        Slig_GameSpeak_SFX(SligSpeak::eBlurgh_11, 0, field_11E_pitch_min, this);
        mCurrentMotion = eSligMotions::Motion_31_Blurgh;
        return 1;
...
s16 Slig::GetNextMotionIncGameSpeak(s32 input)
{
    if (sControlledCharacter == this && mHealth > FP_FromInteger(0))
    {
        ...
        if (InputCommands::Enum::eGameSpeak2 & input)
        {
...
void Slig::PlayerControlRunningSlideStopOrTurnFrame12()
{
    ...
    if (curVelX > FP_FromInteger(0) && Input().IsAnyHeld(InputCommands::Enum::eLeft))
    {
        mVelX = GetSpriteScale() * FP_FromDouble(13.2);
    ...
    if (curVelX < FP_FromInteger(0) && Input().IsAnyHeld(InputCommands::Enum::eRight))
    {
        mVelX = (GetSpriteScale() * -FP_FromDouble(13.2));
        mCurrentMotion = eSligMotions::Motion_9_SlidingTurn;
        mInput = 0;

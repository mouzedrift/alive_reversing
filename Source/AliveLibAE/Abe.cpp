...
void Abe::VUpdate()
{
    ...
    else
    {
        ...
        if (mDoQuicksave)
        {
            ...
            QuikSave::SaveWorldInfo(&QuikSave::gActiveQuicksaveData.mRestartPathWorldInfo, mMap);
            ...
            if (mHealth > FP_FromInteger(0) && mCurrentMotion != eAbeMotions::Motion_71_Knockback_455090)
            {
                ...
                if (mHealth <= FP_FromInteger(0) || (mCurrentMotion != eAbeMotions::Motion_123_LiftGrabIdle && mCurrentMotion != eAbeMotions::Motion_124_LiftUseUp && mCurrentMotion != eAbeMotions::Motion_125_LiftUseDown))
                {
                    ...
                    if (mHealth <= FP_FromInteger(0))
                    {
                        ...
                        Environment_SFX(EnvironmentSfx::eDeathNoise_7, 0, 32767, this, mMap);
                        mHealth = FP_FromInteger(0);
                    }
                }
            }
...
{
    for (; tlvIterator.GetTlv(); tlvIterator = mMap.TLV_Get_At(
                     tlvIterator,
                     mXPos,
                     mYPos,
                     mXPos,
...
void Abe::Motion_0_Idle_44EEB0()
{
    ...
    if (Input().IsAnyHeld(InputCommands::eDown))
    {
        ...
        relive::Path_Hoist* pHoist = static_cast<relive::Path_Hoist*>(mMap.VTLV_Get_At_Of_Type(
            FP_GetExponent(mXPos),
    ...
    if (Input().IsAnyHeld(InputCommands::eUp))
    {
        ...
        // Get the first TLV
        TlvIterator tlvIterator = mMap.TLV_Get_At(
            TlvIterator::Invalid(),
            mXPos,
            mYPos,
            mXPos,
...
void Abe::Motion_1_WalkLoop_44FBA0()
{
    ...
    if (GetAnimation().GetCurrentFrame() != 2 && GetAnimation().GetCurrentFrame() != 11)
    {
        ...
        Environment_SFX(EnvironmentSfx::eWalkingFootstep_1, 0, 32767, this, mMap);
...
void Abe::Motion_2_StandingTurn_451830()
{
    ...
    else
    {
        ...
        if (currentFrame == 0)
        {
            Environment_SFX(EnvironmentSfx::eGenericMovement_9, 0, 32767, this, mMap);
        }
...
void Abe::Motion_3_Fall_459B60()
{
    ...
    if (bCollision)
    {
        switch (pPathLine->mLineType)
        {
            ...
            case eLineTypes::eBackgroundDynamicCollision_36:
            {
                ...
                relive::Path_SoftLanding* pSoftLanding = static_cast<relive::Path_SoftLanding*>(mMap.VTLV_Get_At_Of_Type(
                    FP_GetExponent(mXPos),
                    FP_GetExponent(mYPos),
                    FP_GetExponent(mXPos),
                    FP_GetExponent(mYPos),
    ...
    TlvIterator edgeIterator = mMap.VTLV_Get_At_Of_Type(
                    FP_GetExponent(mXPos),
    ...
    else // Didn't find and edge to grab so check if falling onto a hoist
    {
        ...
        TlvIterator hoistIterator = mMap.VTLV_Get_At_Of_Type(
            FP_GetExponent(mXPos),
...
void Abe::Motion_15_HoistLand_452BA0()
{
    ...
    if (GetAnimation().GetCurrentFrame() == 2)
    {
        ...
        {
            Environment_SFX(EnvironmentSfx::eLandingSoft_5, 0, 32767, this, mMap);
        }
        else
        {
            Environment_SFX(EnvironmentSfx::eHitGroundSoft_6, 0, 32767, this, mMap);
        }
    }
...
void Abe::Motion_23_RollLoop_453A90()
{
    ...
    else
    {
        ...
        if (mCurrentMotion == eAbeMotions::Motion_23_RollLoop_453A90)
        {
            ...
            {
                Environment_SFX(EnvironmentSfx::eRollingNoise_8, 0, 32767, this, mMap);
            }
        }
    }
}
    ...
    else
    {
        ...
        if (mCurrentMotion == eAbeMotions::Motion_25_RunSlideStop_451330 && !RunTryEnterWell_451060() && !RunTryEnterDoor_451220())
        {
            ...
            if (GetAnimation().GetCurrentFrame() >= 9)
            {
                if (GetAnimation().GetCurrentFrame() == 15)
                {
                    Environment_SFX(EnvironmentSfx::eSlideStop_0, 0, 32767, this, mMap);
                    MapFollowMe(true);
...
void Abe::Motion_28_HopMid_451C50()
{
    ...
    else
    {
        ...
        // Check for an edge
        TlvIterator tlvIterator = mMap.VTLV_Get_At_Of_Type(
            FP_GetExponent(mXPos),
            FP_GetExponent(mYPos - (GetSpriteScale() * FP_FromInteger(75))),
            FP_GetExponent(mXPos),
            FP_GetExponent(mYPos),
...
void Abe::Motion_30_RunJumpBegin_4532E0()
{
    ...
    else
    {
        ...
        {
            Environment_SFX(EnvironmentSfx::eRunJumpOrLedgeHoist_11, 0, 32767, this, mMap);
        }
    ...
    if (GetAnimation().GetIsLastFrame())
    {
        ...
        else if (held & InputCommands::eRight)
        {
            ...
            if (GetAnimation().GetFlipX())
            {
                mCurrentMotion = eAbeMotions::Motion_26_RunTurn_451500;
                mVelX = -(ScaleToGridSize(GetSpriteScale()) / FP_FromInteger(4));
                Environment_SFX(EnvironmentSfx::eRunSlide_4, 0, 32767, this, mMap);
            }
...
void Abe::Motion_33_RunLoop_4508E0()
{
    ...
    {
        Environment_SFX(EnvironmentSfx::eRunningFootstep_2, 0, 32767, this, mMap);
...
void Abe::Motion_40_SneakLoop_450550()
{
    ...
    else
    {
        ...
        if (mCurrentMotion == eAbeMotions::Motion_40_SneakLoop_450550)
        {
            ...
            {
                Environment_SFX(EnvironmentSfx::eSneakFootstep_3, 0, 32767, this, mMap);
                MapFollowMe(true);
...
void Abe::Motion_65_LedgeAscend_4548E0()
{
    ...
    if (curFrameNum == 0)
    {
        Environment_SFX(EnvironmentSfx::eExhaustingHoistNoise_10, 0, 32767, this, mMap);
    }
...
void Abe::Motion_83_WellExpressShotOut_45CF70()
{
    ...
    TlvIterator tlvIter = mMap.Get_First_TLV_For_Offsetted_Camera(0, 0);
    ...
    switch (field_120_state.stone)
    {
        case StoneStates::eHandstoneBegin_0:
            if (GetAnimation().GetForwardLoopCompleted())
            {
                ...
                if (!pMovieStoneTlv)
                {
                    TlvIterator handStoneIterator = mMap.VTLV_Get_At_Of_Type(
                        FP_GetExponent(mXPos),
                        FP_GetExponent(mYPos),
                        FP_GetExponent(mXPos),
                        FP_GetExponent(mYPos),
...
void Abe::Motion_114_DoorEnter()
{
    switch (field_120_state.door)
    {
        ...
        case AbeDoorStates::eSetNewActiveCamera_4:
        {
            ...
            static_cast<Map&>(mMap).mDoorTransition = 1;
            s16 bForceChange = 0;
        ...
        {
            static_cast<Map&>(mMap).mDoorTransition = 0;
            mCurrentLevel = mMap.mCurrentLevel;
            mCurrentPath = mMap.mCurrentPath;

            TlvIterator doorTlvIterator = mMap.TLV_First_Of_Type_In_Camera(ReliveTypes::eDoor, 0);
...
void Abe::Motion_115_DoorExit()
{
    if (GetAnimation().GetIsLastFrame())
    {
        ...
        if (static_cast<relive::Path_Door*>(mMap.VTLV_Get_At_Of_Type(
                                        FP_GetExponent(mXPos),
                                        FP_GetExponent(mYPos),
                                        FP_GetExponent(mXPos),
                                        FP_GetExponent(mYPos),
...
void Abe::Motion_127_TurnWheelLoop()
{
    ...
    {
        relive::Path_LevelLoader* pLevelLoader = static_cast<relive::Path_LevelLoader*>(mMap.TLV_First_Of_Type_In_Camera(ReliveTypes::eLevelLoader, 0).GetTlv());
    ...
    else if (field_120_state.wheel == WorkWheelStates::eMapChanging_2)
    {
                ...
                0))
        {
            ...
            auto pPathAbeStart = static_cast<relive::Path_AbeStart*>(mMap.TLV_First_Of_Type_In_Camera(ReliveTypes::eAbeStart, 0).GetTlv());
...
s16 Abe::RunTryEnterDoor_451220()
{
    ...
    TlvIterator doorIterator = mMap.VTLV_Get_At_Of_Type(
        FP_GetExponent(mXPos),
        FP_GetExponent(mYPos),
        FP_GetExponent(mXPos),
        FP_GetExponent(mYPos),
...
void Abe::BulletDamage_44C980(Bullet* pBullet)
{
    ...
    switch (pBullet->mBulletType)
    {
        ...
        case BulletType::eZBullet_3:
        {
            ...
            if (Bullet::InZBulletCover(mMap, mXPos, boundsY, rect) || !mMap.Is_Point_In_Current_Camera(mCurrentLevel, mCurrentPath, mXPos, boundsY, 0))
            {
                mbGotShot = false;
                mHealth = FP_FromInteger(1);
                return;
    ...
    Environment_SFX(EnvironmentSfx::eElumHitWall_14, 0, 32767, this, mMap);

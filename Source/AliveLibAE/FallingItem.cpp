...
{
    if (mMap.mCurrentLevel == EReliveLevelIds::eBonewerkz)
    {
...
FallingItem::FallingItem(relive::Path_FallingItem* pTlv, const Guid& tlvId, ResourceManagerWrapper& resMan, BaseMap& map)
    ...
    mTlvYPos(FP_FromInteger(pTlv->mBottomRightY))
{
    ...
    const s32 lvlIdx = static_cast<s32>(MapWrapper::ToAE(mMap.mCurrentLevel));
 ...
 FallingItem::FallingItem(s32 xpos, s32 ypos, s32 scale, s32 id, s32 fallInterval, s32 numItems, s32 bResetIdAfterUse, ResourceManagerWrapper& resMan, BaseMap& map)
    : BaseAliveGameObject(0, resMan, map),
     mTlvId(Guid{}),
    ...
    switch (mState)
    {
        case State::eWaitForIdEnable_0:
            if (mSwitchId && SwitchStates_Get(mSwitchId))
            {
                ...
                GetAnimation().Set_Animation_Data(GetAnimRes(sFallingItemData[static_cast<s32>(MapWrapper::ToAE(mMap.mCurrentLevel))][1]));
        ...
        case State::eFalling_3:
        {
            ...
            relive_new ScreenShake(false, GetSpriteScale() == FP_FromDouble(0.5), mResMan, mMap);
        ...
        case State::eSmashed_4:
            ...
            if ((mMaxFallingItems > 0 && mRemainingFallingItems <= 0) || !mMap.Is_Point_In_Current_Camera(mCurrentLevel, mCurrentPath, mTlvXPos, mTlvYPos, 0))
            {
                SetDead(true);
            }
            else
            {
                GetAnimation().Set_Animation_Data(GetAnimRes(sFallingItemData[static_cast<s32>(MapWrapper::ToAE(mMap.mCurrentLevel))][0]));
                SetCanExplode(true);
                mVelY = FP_FromInteger(0);
                mVelX = FP_FromInteger(0);
                mYPos = mStartYPos;

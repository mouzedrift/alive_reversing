...
Slog::Slog(FP xpos, FP ypos, FP scale, s16 bListenToSligs, s16 chaseDelay, ResourceManagerWrapper& resMan, BaseMap& map)
    : BaseAliveGameObject(5, resMan, map)
{
    LoadAnimations();
    ...
    if (createParticle)
    {
        if (GetMap().Is_Point_In_Current_Camera(mCurrentLevel, mCurrentPath, mXPos, mYPos, 0))
        {
            ...
            relive_new SnoozeParticle(xOff + mXPos,
                                                          ...
                                                          GetAnimation().GetRenderLayer(), GetAnimation().GetSpriteScale(), mResMan, mMap);
        }
    }
}
...
void Slog::Motion_20_Eating()
{
    ...
    if (GetAnimation().GetCurrentFrame() == 3 && !GetAnimation().GetLoopBackwards())
    {
        ...
        relive_new Blood(((GetAnimation().GetFlipX()) != 0 ? FP_FromInteger(-25) : FP_FromInteger(25)) * GetSpriteScale() + mXPos,
                      ...
                      GetSpriteScale(), 12, mResMan, mMap);
    }

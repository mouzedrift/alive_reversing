...
void Fleech::CreateFromSaveState(SerializedObjectData& pBuffer, ResourceManagerWrapper& resMan, BaseMap& map)
{
    ...
    auto pTlv = map.TLV_From_Offset_Lvl_Cam(pState->mTlvInfo).GetTlv<relive::Path_Fleech>();
...
void Fleech::Motion_11_RaiseHead()
{
    if (GetAnimation().GetIsLastFrame())
    {
        ...
        auto pHoist = mMap.VTLV_Get_At_Of_Type(
            mHoistX,
        ...
        if (pHoist->mHoistType == relive::Path_Hoist::Type::eOffScreen)
        {
            ...
            pHoist = mMap.VTLV_Get_At_Of_Type(
                mHoistX,
...
void Fleech::Init()
{
    ...
    SetTint(&kFleechTints_551844[0], mMap.mCurrentLevel);
...
s16 Fleech::HandleEnemyStopperOrSlamDoor(s32 velX)
{
    ...
    auto pSlamDoor = mMap.VTLV_Get_At_Of_Type(
...
void Fleech::IncreaseAnger()
{
    if (mMap.Is_Point_In_Current_Camera(mCurrentLevel, mCurrentPath, mXPos, mYPos, 0))
    {
        ...
        if (pEvent)
        {
            if ((!IsAbe(pEvent) || !gAbe->GetInvisible()) && mMap.Is_Point_In_Current_Camera(mCurrentLevel, mCurrentPath, pEvent->mXPos, pEvent->mYPos, 0))
            {
...
ChasingAbeBrain::EState ChasingAbeBrain::Brain_ChasingAbe_State_1(BaseAliveGameObject* pObj)
{
    ...
    if (mFleech.mVelX != FP_FromInteger(0))
    {
        ...
        if (mFleech.HandleEnemyStopperOrSlamDoor(1))
        {
            ...
            relive::Path_TLV* pSlamDoor = mFleech.GetMap().VTLV_Get_At_Of_Type(

    ...
    : BaseAliveGameObject(9)
{
    ...
    {
        mTimer = sGnFrame + field_118_data.mSpawnMoveDelay;
    }
    ...
    mMax_XSpeed = FP_FromInteger(field_118_data.mMaxVelocity) * GetSpriteScale();
    mUpVel = FP_FromInteger(-field_118_data.mMaxVelocity) * GetSpriteScale();
    mDownVel = FP_FromInteger(field_118_data.mMaxVelocity) * GetSpriteScale();
    mMaxSlow = FP_FromDouble(0.4) * GetSpriteScale();
    mMaxSpeedUp = FP_FromDouble(0.4) * GetSpriteScale();
...
s32 FlyingSlig::CreateFromSaveState(const u8* pBuffer)
{
    ...
    if (pFlyingSlig)
    {
        ...
        pFlyingSlig->mHealth = pSaveState->mCurrentHealth;
        ...
        if (pSaveState->mPossessed)
        {
            ...
            pFlyingSlig->mMax_XSpeed = (FP_FromDouble(5.5) * pFlyingSlig->GetSpriteScale());
            pFlyingSlig->mUpVel = (FP_FromDouble(-5.5) * pFlyingSlig->GetSpriteScale());
            pFlyingSlig->mDownVel = (FP_FromDouble(5.5) * pFlyingSlig->GetSpriteScale());
            pFlyingSlig->mMaxSlow = (FP_FromDouble(0.25) * pFlyingSlig->GetSpriteScale());
            pFlyingSlig->mMaxSpeedUp = (FP_FromDouble(0.7) * pFlyingSlig->GetSpriteScale());
        }
        ...
        pFlyingSlig->mTimer = pSaveState->field_40_timer;
        pFlyingSlig->mGrenadeDelay = pSaveState->field_44_grenade_delay;
        pFlyingSlig->mCollisionReactTimer = pSaveState->field_48_collision_reaction_timer;
...
s32 FlyingSlig::VGetSaveState(u8* pSaveBuffer)
{
    ...
    pState->field_40_timer = mTimer;
    pState->field_44_grenade_delay = mGrenadeDelay;
    pState->field_48_collision_reaction_timer = mCollisionReactTimer;
...
void FlyingSlig::Movement()
{
    if (!IsPossessed())
    {
        ...
        {
            if (field_18C + field_190 > mMax_XSpeed)
            {
                field_18C = mMax_XSpeed;
            }
            else
            {
                field_18C += field_190;
    ...
    else
    {
        ...
        {
            mVelX = mVelX - ((mVelX / FP_FromInteger(newVel)) * mMaxSlow) + field_184_xSpeed;
            mVelY = mVelY - ((mVelY / FP_FromInteger(newVel)) * mMaxSlow) + field_188_ySpeed;
        }
        else
        {
        ...
        if (v15 > mMax_XSpeed)
        {
            mVelX = ((mVelX / v15) * mMax_XSpeed);
            mVelY = ((mVelY / v15) * mMax_XSpeed);
        }
    ...
    if (mThrowGrenade)
    {
        if (static_cast<s32>(sGnFrame) > mGrenadeDelay && 
    ...
    const s32 v38 = static_cast<s32>(sGnFrame) % ((FP_FromInteger(v37) < mMax_XSpeed) + 2);
...
void FlyingSlig::Brain_3_GetAlerted()
{
    ...
    {
        if (!sub_436730() && static_cast<s32>(sGnFrame) >= mTimer)
        {
            ToMoving();
        }
    }
...
void FlyingSlig::Brain_4_ChasingEnemy()
{
    ...
    if (sub_436C60(&BaseAliveGameObjectCollisionLine->mRect, 1))
    {
        ...
        if (static_cast<s32>(sGnFrame) > mGrenadeDelay && CanThrowGrenade())
        {
...
void FlyingSlig::Brain_8_PanicIdle()
{
    ...
    {
        if (static_cast<s32>(sGnFrame) >= mTimer && GetCurrentMotion() != eFlyingSligMotions::Motion_8_GameSpeak)
        {
            ToPanicMoving();
        }
    }
...
{
    if (static_cast<s32>(sGnFrame) == mTimer - 45)
    {
...
void FlyingSlig::ToPanicIdle()
{
    ...
    mTimer = (Math_NextRandom() & 7) + sGnFrame + field_118_data.mPanicDelay;
...
s16 FlyingSlig::sub_4374A0(s16 a2)
{
    ...
    if (mUnknown2)
    {
        if (!mNoNextLine && (field_182_bound1 == ReliveTypes::eNone || !a2))
        {
            if (!mUnknown1)
            {
                field_190 = mMaxSpeedUp;
            }
            else
            {
                const FP v64 = FP_Abs(((field_18C * field_18C) / mMaxSlow) * FP_FromDouble(0.5));
        ...
        {
            FP directedXMaxSpeedUp = mMaxSpeedUp + field_18C;
            if (directedXMaxSpeedUp > mMax_XSpeed)
            {
                directedXMaxSpeedUp = mMax_XSpeed;
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    const FP v65 = FP_Abs((((field_18C * field_18C) / mMaxSlow) * FP_FromDouble(0.5)));
                    const FP v27 = field_198_line_length - field_194;
                    if (v27 < mMax_XSpeed && field_18C == FP_FromInteger(0))
                    {
                        return 1;
                    }
    ...
    else
    {
        if (!mNoPrevLine && (field_180_bound2 == ReliveTypes::eNone || !a2))
        {
            ...
            {
                const FP v66 = FP_Abs(((field_18C * field_18C) / mMaxSlow) * FP_FromDouble(0.5));
        ...
        {
            FP directedXMaxSpeed = -mMax_XSpeed;
            if (field_18C - mMaxSpeedUp >= directedXMaxSpeed)
            {
                directedXMaxSpeed = field_18C - mMaxSpeedUp;
            }
            ...
            else
            {
                ...
                {
                    mLastLine = true;
                    const FP v2 = FP_Abs(((field_18C * field_18C) / mMaxSlow) * FP_FromDouble(0.5));
                    if (field_194 < mMax_XSpeed && field_18C == FP_FromInteger(0))
                    {
                        return 1;
                    }
...
void FlyingSlig::ThrowGrenade()
{
    ...
    mGrenadeDelay = randomisedGrenadeDelay + sGnFrame;
...
void FlyingSlig::ToSpottedEnemy()
{
    if (field_118_data.mPrechaseDelay)
    {
        ...
        mTimer = (Math_NextRandom() & 7) + sGnFrame + field_118_data.mPrechaseDelay;
...
void FlyingSlig::ToAlerted()
{
    ...
    mTimer = sGnFrame + field_118_data.mAlertedListenTime;
}

void FlyingSlig::ToPanicMoving()
{
    MusicController::static_PlayMusic(MusicController::MusicTypes::eSoftChase_8, this, 0, 0);
    mTimer = (Math_NextRandom() & 7) + sGnFrame + 12;
...
void FlyingSlig::PatrolDelay()
{
    ...
    {
        mTimer = (Math_NextRandom() & 7) + sGnFrame + field_118_data.mGiveUpChaseDelay;
        SetBrain(&FlyingSlig::Brain_5_Idle);
        return;
    }

    mTimer = field_118_data.mPatrolPauseMin + sGnFrame;
    ...
    mTimer += Math_NextRandom() % (field_118_data.mPatrolPauseMin - field_118_data.mPatrolPauseMax);
    SetBrain(&FlyingSlig::Brain_5_Idle);
}
...
void FlyingSlig::HandlePlayerControls()
{
    ...
    */
    ...
    if (Input().isPressed(InputCommands::Enum::eUp))
    {
        ...
        {
            field_184_xSpeed = (-mMaxSpeedUp * FP_FromDouble(0.707));
            field_188_ySpeed = (-mMaxSpeedUp * FP_FromDouble(0.707));
            return;
        }
...
void FlyingSlig::VPossessed()
{
    ...
    mMax_XSpeed = FP_FromDouble(5.5) * GetSpriteScale();
    mUpVel = FP_FromDouble(-5.5) * GetSpriteScale();
    mDownVel = FP_FromDouble(5.5) * GetSpriteScale();
    mMaxSlow = FP_FromDouble(0.25) * GetSpriteScale();
    mMaxSpeedUp = FP_FromDouble(0.7) * GetSpriteScale();

...
SligSpawner::SligSpawner(relive::Path_Slig* pTlv, const Guid& tlvId, ResourceManagerWrapper& resMan, BaseMap& map)
...
{
    if (GetMap().LevelChanged() || GetMap().PathChanged() || mState == SpawnerStates::eInactive_0)
    {
        SetDead(true);
    }
}

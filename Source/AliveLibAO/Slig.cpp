...
s16 Slig::HandlePlayerControlled()
{
    ...
    if (Input().IsAnyHeld(InputCommands::eHop | InputCommands::eCrouchOrRoll) && !(Input().IsAnyPressed(InputCommands::eLeftGameSpeak | InputCommands::eRightGameSpeak)))
    {
...
{
    if (mVelX > FP_FromInteger(0) && Input().IsAnyPressed(InputCommands::eLeft))
    {
    ...
    if (mVelX < FP_FromInteger(0) && Input().IsAnyPressed(InputCommands::eRight))
    {
        mVelX = GetSpriteScale() * FP_FromDouble(-13.2);
        mCurrentMotion = eSligMotions::Motion_10_SlidingTurn;
        field_126_input = 0;
    ...
    if (mCurrentMotion == eSligMotions::Motion_2_Walking)
    {
        ...
        switch (GetAnimation().GetCurrentFrame())
        {
            ...
            case 11:
                ...
                {
                    if ((mVelX > FP_FromInteger(0) && Input().IsAnyPressed(InputCommands::eLeft))
                        || (mVelX < FP_FromInteger(0) && Input().IsAnyPressed(InputCommands::eRight))
                        || !(Input().IsAnyPressed(InputCommands::eRight | InputCommands::eLeft)))
                    {
                        mCurrentMotion = eSligMotions::Motion_19_WalkToStand;
                    }
                    field_126_input = 0;
    ...
    if (GetAnimation().GetIsLastFrame())
    {
        if (sControlledCharacter == this && mHealth > FP_FromInteger(0))
        {
            if (Input().IsAnyPressed(InputCommands::eThrowItem))
            {
                ...
                // Recoil right
                if (GetAnimation().GetFlipX() && Input().IsAnyPressed(InputCommands::eRight))
                {
                    PathLine* pLine = nullptr;
                    FP hitX = {};
                    FP hitY = {};
                ...
                // Recoil left
                if (!GetAnimation().GetFlipX() && Input().IsAnyPressed(InputCommands::eLeft))
                {
                    PathLine* pLine = nullptr;
                    FP hitX = {};
                    FP hitY = {};
...
void Slig::Motion_9_SlidingToStand()
{
    ...
    else
    {
        ...
        if (mCurrentMotion == eSligMotions::Motion_9_SlidingToStand)
        {
            ...
            else if ((GetAnimation().GetFlipX() && Input().IsAnyPressed(InputCommands::eRight))
                     || (!(GetAnimation().GetFlipX()) && Input().IsAnyPressed(InputCommands::eLeft)))
            {
...
void Slig::Motion_16_StandingToStep()
{
    if (GetAnimation().GetIsLastFrame())
    {
        if (Input().IsAnyPressed(InputCommands::eLeft | InputCommands::eRight))
        {
            mCurrentMotion = eSligMotions::Motion_1_StandToWalk;
        }
...
void Slig::Motion_20_Recoil()
{
    ...
    if (mCurrentMotion == eSligMotions::Motion_20_Recoil)
    {
        if (GetAnimation().GetIsLastFrame())
        {
            ...
            else if (Input().IsAnyPressed(InputCommands::eThrowItem))
            {
                mVelX = FP_FromInteger(0);

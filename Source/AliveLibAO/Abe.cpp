...
Abe::~Abe()
{
    ...
    auto pThrowable1 = sObjectIds.Find_Impl(mSlapableOrPickupId);
    if (pThrowable1)
    {
    ...
    auto pThrowable2 = sObjectIds.Find_Impl(mThrowableId);
    if (pThrowable2)
    {
...
void Abe::VUpdate()
{
    ...
    else
    {
        ...
        if (!mShrivel)
        {
            ...
            if (mSay >= 0 && static_cast<s32>(sGnFrame) >= mAutoSayTimer)
            {
                        ...
                        0))
                {
                        ...
                        && mCurrentMotion != eAbeMotions::Motion_14_Speak)
                    {
                        ...
                        Mudokon_SFX(static_cast<MudSounds>(mSay), 0, 0, this);
                    }
                }
                mSay = -1;
            }
...
void Abe::ChangeChantState(bool bKeepChanting)
{
    if (bKeepChanting)
    {
        mStatesUnion.chant = ChantStates::eChantingForBirdPortal_6;
    }
    else
    {
...
void Abe::ToKnockback(bool bKnockbackSound, bool bDelayedAnger)
{
    if (sControlledCharacter->Type() != ReliveTypes::eSlig || mHealth <= FP_FromInteger(0))
    {
        ...
        auto pThrowable = static_cast<::BaseThrowable*>(sObjectIds.Find_Impl(mThrowableId));
        if (pThrowable)
        {
            pThrowable->VToDead();
            mThrowableId = Guid{};
        ...
        if (pObj->GetInteractive())
        {
            ...
            if (fpX >= pAliveObj->mCollectionRect.x && fpX <= pAliveObj->mCollectionRect.w)
            {
                ...
                if (yPos >= pAliveObj->mCollectionRect.y && yPos <= pAliveObj->mCollectionRect.h)
                {
                    ...
                    mSlapableOrPickupId = pSlapableOrCollectable->mBaseGameObjectId;
                    break;
                }
            }
        }
    ...
    if (pSlapableOrCollectable)
    {
        ...
        switch (pSlapableOrCollectable->Type())
        {
            ...
            case ReliveTypes::eRock:
            {
                ...
                auto pThrowable = static_cast<::BaseThrowable*>(sObjectIds.Find_Impl(mSlapableOrPickupId));
                mBaseThrowableCount += static_cast<s8>(pThrowable->VGetCount());
...
void Abe::CrouchingGameSpeak()
{
    ...
    {
        if (mPrevInput & InputCommands::eGameSpeak2)
        {
            gEventSystem->PushEvent(GameSpeakEvents::eAbe_FollowMe);
            Mudokon_SFX(MudSounds::eFollowMe_4, 0, 0, this);
        ...
        else if (mPrevInput & InputCommands::eGameSpeak3)
        {
            gEventSystem->PushEvent(GameSpeakEvents::eAbe_Anger);
            Mudokon_SFX(MudSounds::eAngry_5, 0, 0, this);
            mCurrentMotion = eAbeMotions::Motion_23_CrouchSpeak;
    ...
    {
        if (mPrevInput & InputCommands::eGameSpeak6)
        {
            gEventSystem->PushEvent(GameSpeakEvents::eAbe_WhistleHigh);
            Mudokon_SFX(MudSounds::eWhistleHigh_1, 0, 0, this);
            mCurrentMotion = eAbeMotions::Motion_23_CrouchSpeak;
    ...
    while (pTlv)
    {
        if (pTlv->mTlvType == ReliveTypes::eContinuePoint)
        {
            ...
            if ((pContinuePointTlv->mZoneNumber != mContinuePointZoneNumber || mContinuePointLevel != gMap.mCurrentLevel) && !GetElectrocuted() && mCurrentMotion != eAbeMotions::Motion_156_DoorEnter)
            {
                ...
                mSavedRingTimer = bHaveShry < 0 ? 0 : bHaveShry;
                mSavedHaveShrykull = mHaveShrykull;
    ...
    switch (pFrom->Type())
    {
        case ReliveTypes::eBat:
            if (mHealth > FP_FromInteger(0))
            {
                ...
                else
                {
                    ...
                    mHealth = FP_FromInteger(0);
                    mbGotShot = true;
                    mKnockdownMotion = eAbeMotions::Motion_128_KnockForward;
        ...
        case ReliveTypes::eAirExplosion:
            if (GetAnimation().GetRender())
            {
                ...
                {
                    mbGotShot = true;
                    mKnockdownMotion = eAbeMotions::Motion_70_Knockback;
        ...
        case ReliveTypes::eSecurityOrb:
            ...
            for (s32 i = 0; i < mBaseThrowableCount; i++)
            {
                ...
                mThrowableId = pThrowable->mBaseGameObjectId;
        ...
        case ReliveTypes::eSlog:
            if (mHealth > FP_FromInteger(0))
            {
                ...
                {
                    mNextMotion = eAbeMotions::Motion_148_Shot;
                    mKnockdownMotion = eAbeMotions::Motion_148_Shot;
                    mbGotShot = true;
                    return true;
                }
        ...
        case ReliveTypes::eZBall:
            if (mHealth > FP_FromInteger(0))
            {
                ...
                {
                    mKnockdownMotion = eAbeMotions::Motion_147_ShotRolling;
                    mNextMotion = eAbeMotions::Motion_147_ShotRolling;
                }
            }
            break;
...
void Abe::Motion_0_Idle()
{
        ...
        while (pTlv)
        {
            switch (pTlv->mTlvType)
            {
                ...
                case ReliveTypes::eHandStone:
                {
                    ...
                    mStatesUnion.stone = StoneStates::eHandstoneStart_0;
                    return;
                }
                case ReliveTypes::eBoomMachine:
                {
    ...
    if (!Input().IsAnyHeld(InputCommands::eUp) || handleDoActionOrThrow)
    {
        ...
        {
            if (mBaseThrowableCount > 0 || gInfiniteGrenades)
            {
...
void Abe::Motion_19_CrouchIdle()
{
    ...
    if (Input().IsAnyPressed(InputCommands::eThrowItem))
    {
        ...
        if (!mBaseThrowableCount && !gInfiniteGrenades)
        {
            Mudokon_SFX(MudSounds::eDunno_15, 0, 0, this);
            mCurrentMotion = eAbeMotions::Motion_23_CrouchSpeak;
            return;
        ...
        if (mCurrentMotion == eAbeMotions::Motion_25_RollLoop)
        {
            if (GetAnimation().GetCurrentFrame() == 1 || GetAnimation().GetCurrentFrame() == 5 || GetAnimation().GetCurrentFrame() == 9)
            {
                    ...
                    || field_12C_timer + 9 >= static_cast<s32>(sGnFrame))
                {
                    ...
                    // AE
                    if (mReleasedButtons && InputCommands::eCrouchOrRoll)
                    {
...
void Abe::IntoPortalStates()
{
    switch (mBirdPortalSubState)
    {
        ...
        case PortalSubStates::eHopOutOfPortal_2:
        {
            ...
            {
                mBirdPortalId->ClipPortal(0);
                GetAnimation().SetRender(true);
                mCurrentMotion = eAbeMotions::Motion_29_HopBegin;
                mBirdPortalId->RemovePortalClipperDelayed();
                mBirdPortalId = 0;
            }
            break;
        }
        case PortalSubStates::eSetNewAbePosition_4:
        {
            ...
            if (GetAnimation().GetFlipX())
            {
                mXPos = ScaleToGridSize(GetSpriteScale()) + mBirdPortalId->mExitX;
            }
            else
            {
                mXPos = mBirdPortalId->mExitX - ScaleToGridSize(GetSpriteScale());
            }

            mYPos = mBirdPortalId->mExitY;
            mVelY = FP_FromInteger(0);
            mFallMotionVelX = FP_FromInteger(0);
            break;
        }
...
void Abe::Motion_35_RunLoop()
{
    ...
    else if (GetAnimation().GetCurrentFrame() == 4 || GetAnimation().GetCurrentFrame() == 12)
    {
        ...
        // Running to roll
        if (mPrevInput & InputCommands::eCrouchOrRoll)
        {
...
void Abe::Motion_58_ToSpeak()
{
    ...
    if (GetAnimation().GetIsLastFrame())
    {
        mCurrentMotion = DoGameSpeak(mPrevInput);
        ...
        case 0:
        {
            ...
            if (static_cast<s32>(sGnFrame) > field_118_timer)
            {
                ...
                if (mBaseThrowableCount)
                {
                    ...
                    gThrowableArray->Add(mBaseThrowableCount);
                }
                field_114_gnFrame = 1;
                mCurrentMotion = eAbeMotions::Motion_61_Respawn;
            }
...
void Abe::Motion_70_Knockback()
{
    ...
    {
        mBirdPortalId->DestroyPortalClippers();
        mBirdPortalId = nullptr;
    }

    if (GetAnimation().GetCurrentFrame() == 12)
    {
        CollideWithUxbOrMine();
    }
...
void Abe::Motion_81_InsideWellExpress()
{
    ...
    {
        if (mDstWellLevel == EReliveLevelIds::eForest)
        {
            if (mParamoniaDone)
            {
                mWalkToRun = true;
        ...
        if (mDstWellLevel == EReliveLevelIds::eDesert)
        {
            if (mScrabaniaDone)
            {
                mWalkToRun = true;
    ...
    if (mDstWellLevel != gMap.mCurrentLevel
        || mDstWellPath != gMap.mCurrentPath
...
void Abe::Motion_82_WellExpressShotOut()
{
    ...
    do
    {
        ...
        if ((pTlvIter->mTlvType == ReliveTypes::eWellLocal || pTlvIter->mTlvType == ReliveTypes::eWellExpress) && static_cast<relive::Path_WellBase*>(pTlvIter)->mOtherWellId == mObjectIdInCam)
        {
    ...
    switch (mStatesUnion.stone)
    {
            ...
            if (pCircularFade->VDone())
            {
                switch (mHandStoneType)
                {
                    ...
                    case ReliveTypes::eBellSongStone:
                    {
                        ...
                        mStatesUnion.stone = StoneStates::eBellSongDone_4;
                        break;
                    }
                    case ReliveTypes::eDemoPlaybackStone:
                        ALIVE_FATAL("never expected eDemoPlaybackStone to be used");
                    ...
                    {
                        GetAnimation().SetRender(false);
                        mStatesUnion.stone = StoneStates::eWaitForInput_6;
                        mHandStoneCamIdx = 1;
                        pCircularFade->SetDead(true);
                        mCircularFadeId = Guid{};
        ...
        case StoneStates::eHandstoneMovieDone_2:
        {
            if (gMovieRefCount == 0)
            {
                ...
                mStatesUnion.stone = StoneStates::eHandstoneEnd_5;
            }
            break;
        }
        ...
        case StoneStates::eWaitForInput_6:
        {
            ...
            if (pFade && pFade->mDone)
            {
                if (Input().IsAnyPressed(InputCommands::eHop | InputCommands::eThrowItem | InputCommands::eCrouchOrRoll | InputCommands::eDoAction))
                {
                    ...
                    mStatesUnion.stone = StoneStates::eSetActiveCamToAbeOrWaitForInput_7;
                    SfxPlayMono(relive::SoundEffects::IngameTransition, 90);
                }
            }
            break;
        ...
        case StoneStates::eSetActiveCamToAbeOrWaitForInput_7:
        {
            ...
            if (pFade && pFade->mDone)
            {
                ...
                switch (mHandStoneCamIdx)
                {
                    ...
                    default:
                        LOG_ERROR("CameraIdx out of bounds %d", mHandStoneCamIdx);
                }
                if (mHandStoneCamIdx > 2 || (camera.level == LevelIds::eForestChase_14 || camera.level == LevelIds::eDesertEscape_15))
                {
                    mStatesUnion.stone = StoneStates::eSetActiveCamToAbe_12;
                }
                else
                {
                    pFade->SetDead(true);
        ...
        case StoneStates::eSetActiveCamToAbe_12:
        {
            ...
            {
                GetAnimation().SetRender(true);
                mStatesUnion.stone = StoneStates::eCircularFadeExit_13;
                gMap.SetActiveCam(mDstWellLevel, mDstWellPath, mDstWellCamera, CameraSwapEffects::eInstantChange_0, 0, 0);
            }
            break;
        }
...
void Abe::Motion_156_DoorEnter()
{
    switch (mStatesUnion.door)
    {
        ...
        case AbeDoorStates::eWaitABit_2:
        {
            ...
            {
                mStatesUnion.door = AbeDoorStates::eClearTlvIds_3;
                field_118_timer = MakeTimer(3);
            }
            return;
        }
        ...
        case AbeDoorStates::eSetNewAbePosition_5:
        {
            ...
            while (pPathDoor->mDoorId != mObjectIdInCam)
            {
